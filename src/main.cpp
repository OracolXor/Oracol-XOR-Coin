...
// Copyright (c) 2017 The Oracol Developers
...
#if defined(NDEBUG)
# error "Oracol cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x991d4ce275daa746dd529bc44ef6ef6150c1db7b74e4a02a837e49583a8a31a7");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Oracol starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Oracol Signed Message:\n";
...
{
    // Oracol: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Oracol: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Oracol: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Oracol: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Oracol: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x991d4ce275daa746dd529bc44ef6ef6150c1db7b74e4a02a837e49583a8a31a7");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "24/May/1775 John Hancokc becomes president.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xddf6d1a3053fcea164fbae5c9118f92e0eafcfe1a91e530686063a0f18cfdffe"));
...
unsigned char pchMessageStart[4] = { 0x78, 0x6f, 0x72, 0xdb }; // Oracol: increase each by adding 2 to oracol's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OracolMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OracolMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OracolMiner(CWallet *pwallet)
{
    printf("OracolMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("oracol-miner");
    ...
    try { loop {
        ...
        printf("Running OracolMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OracolMiner, pwallet));
}
